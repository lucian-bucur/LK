...
class ZabbixInventory(object):
    ...
    def read_settings(self):
        ...
        #http login
        if config.has_option('zabbix', 'http_username'):
            self.zabbix_http_username = config.get('zabbix', 'http_username')
        if config.has_option('zabbix', 'http_password'):
            self.zabbix_http_password = config.get('zabbix', 'http_password')
    ...
    def __init__(self):
        ...
        self.zabbix_server = 'https://monitor.seinternal.com/zabbix/api_jsonrpc.php'
        ...
        if self.zabbix_server and self.zabbix_username:
            try:
                api = ZabbixAPI(server=self.zabbix_server, user=self.zabbix_http_username, passwd=self.zabbix_http_password)
